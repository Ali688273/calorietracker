...
const DEFAULT={lastClaim:'',streak:0,totalDays:0,lastAdDate:'',adsToday:0,claimed:{},version:1};
...
function watchAd(){
 const d=day();
 if(st.lastAdDate!==d){st.lastAdDate=d;st.adsToday=0}
 if(st.adsToday>=MAX_ADS_PER_DAY){alert('سقف پاداش تبلیغ امروز تکمیل شده است.');return false}
 const grant=()=>{st.adsToday++;coin(AD_REWARD,'تماشای تبلیغ جایزه‌ای');save();render()};
...
function render(){
 ...
 '<div style="font-size:.78rem;color:var(--text-sub)">تبلیغ جایزه‌ای: '+st.adsToday+'/'+MAX_ADS_PER_DAY+' · هر تبلیغ: '+AD_REWARD+' سکه</div>'+
